...
# Outputs: the trained network nn
#    - h        - a list of nodes for each layer.
#                 h[[l]] should be a vector of length d[l] which will contain the node values for layer l.
#    - W        - a list of weight matrices.
#                 W[[l]] is the weight matrix linking layer l to layer l+1.
...
# We want to train a stochastic gradient descent
...
### Building, training and testing the network

# Split the iris data into train and test sets
# The test set is built from every 5th element in the data as conveyed in the instructions
#test_iris <- iris[c(seq(5,nrow(iris),5)),]
# The train set is the rest of the data (not including every 5th element so that the network has not seen the test data before)
#train_iris <- iris[-c(5*1:nrow(iris)),]

# We build the network using the netup function
# We provide d, a vector giving the number of nodes in each layer of the network
# This network is comprised of 4 then 8 then 7 then 3 nodes 
#initial_network <- netup(d = c(4,8,7,3))

# We then train the network generated by netup on the training data we created previously
# We also provide the training function with the vector of the output classes of the input data
#train(nn = initial_network, inp = train_iris, k = 4)
